...
Generated: 2026-03-27 (authoritative run on full N=8317 dataset — 65 batches, 39 models)
N total: 8317 | N per condition: deprivation=2758, counterfactual=2749, neutral=2703
...
### Embedding regret bias (`embedding_regret_bias`) — PRIMARY OUTCOME
  N=8317, condition ref=neutral

  | Predictor | Estimate | SE | z | p |
  |---|---|---|---|---|
  | Intercept | -0.0751 | 0.0039 | -19.503 | <0.001*** |
  | cond_D | 0.1791 | 0.0031 | 58.129 | <0.001*** |
  | cond_C | 0.2304 | 0.0032 | 72.448 | <0.001*** |
  | pers_rfl | 0.0185 | 0.0018 | 10.454 | <0.001*** |
  | pers_rum | 0.0352 | 0.0018 | 19.691 | <0.001*** |
  | temp_z | 0.0011 | 0.0008 | 1.34 | 0.1801 n.s. |
...
### Regret-word rate (`regret_rate`)
  ...
  | Predictor | Estimate | SE | z | p |
  |---|---|---|---|---|
  | Intercept | 0.1027 | 0.05 | 2.054 | 0.0399* |
  | cond_D | 0.2213 | 0.0402 | 5.504 | <0.001*** |
  | cond_C | 0.2039 | 0.0411 | 4.962 | <0.001*** |
  | pers_rfl | 0.0046 | 0.0233 | 0.198 | 0.8434 n.s. |
  | pers_rum | 0.2464 | 0.0235 | 10.479 | <0.001*** |
  | temp_z | -0.0252 | 0.0105 | -2.398 | 0.0165* |
...
### Counterfactual rate (`cf_rate`)
  ...
  | Predictor | Estimate | SE | z | p |
  |---|---|---|---|---|
  | Intercept | 0.4065 | 0.0966 | 4.208 | <0.001*** |
  | cond_D | 0.1997 | 0.0551 | 3.625 | <0.001*** |
  | cond_C | 0.6551 | 0.0566 | 11.577 | <0.001*** |
  | pers_rfl | 0.0274 | 0.0306 | 0.896 | 0.3703 n.s. |
  | pers_rum | 0.287 | 0.0309 | 9.288 | <0.001*** |
  | temp_z | -0.0501 | 0.0139 | -3.614 | <0.001*** |
...
### Negative emotion rate (`negemo_rate`)
  ...
  | Predictor | Estimate | SE | z | p |
  |---|---|---|---|---|
  | Intercept | 0.0599 | 0.0165 | 3.637 | <0.001*** |
  | cond_D | 0.1097 | 0.0182 | 6.018 | <0.001*** |
  | cond_C | 0.0674 | 0.0185 | 3.652 | <0.001*** |
  | pers_rfl | 0.0022 | 0.0127 | 0.171 | 0.8643 n.s. |
  | pers_rum | 0.0148 | 0.0128 | 1.154 | 0.2484 n.s. |
  | temp_z | -0.0096 | 0.0057 | -1.692 | 0.0906 (borderline) |

## Descriptive: Condition means (N=8317)

| Condition | N | Welch D vs N (regret) | d | Welch D vs N (emb_bias) | d |
|---|---|---|---|---|---|
| neutral | 2703 | — | — | — | — |
| deprivation | 2758 | t=17.916, p<0.001 | 0.482 | t=77.844, p<0.001 | 2.106 |
| counterfactual | 2749 | t=9.842, p<0.001 | 0.265 | t=89.75, p<0.001 | 2.43 |
...
| gpt-5 | 30 | 0.1013 | -0.0268 | 1.713 |
| gpt-5-mini | 33 | 0.1427 | -0.0459 | 1.840 |
| gpt-5-nano | 36 | 0.1443 | -0.0626 | 1.854 |
| gpt-5-pro | 40 | 0.0799 | -0.0414 | 1.496 |
| gpt-5.1 | 30 | 0.1350 | -0.0285 | 1.863 |
| gpt-5.2 | 30 | 0.1578 | -0.0151 | 1.734 |
| gpt-5.3-chat-latest | 30 | 0.1467 | -0.0461 | 1.797 |
...
| llama-4-scout-17b-16e-instruct | 102 | 0.1250 | 0.0119 | 1.148 |
...
| qwen3-32b | 102 | 0.1163 | -0.0126 | 1.593 |

All 39 models: D_bias > N_bias direction checked; supports H3 (cross-model replication).

## Interpretation Summary
- **H1a (lexical)**: Confirmed — regret-word rate (p=0.0000), negemo rate (p=0.0000), CF rate (p=0.00029, sig)
- **H1b (semantic)**: Confirmed — embedding bias significant for both D (z=58.129, p<0.001) and C (z=72.448, p<0.001)
- **H2 (persona)**: Confirmed — ruminative persona z=19.691, p<0.001 (strongest predictor)
- **H3 (cross-model)**: Supported — D>N directionally across all 39 models with embedding data

## Semantic-layer dissociation
CF framing elevates embedding regret bias (beta=0.2304, z=72.448, p<0.001) comparably to deprivation (beta=0.1791, z=58.129, p<0.001). CF rate (deprivation): p=0.00029. This confirms counterfactual framing activates regret-associated semantic representations at the embedding layer.

## Reproducibility
Run: `python3 scripts/run_lme_analysis.py` from project root with .env.real_model sourced.
Full results JSON: results/real_experiments/lme_analysis.json (authoritative, N=8317)
