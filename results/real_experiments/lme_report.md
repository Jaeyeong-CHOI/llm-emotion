...
### Embedding regret bias (`embedding_regret_bias`) — PRIMARY OUTCOME
  N=7957, condition ref=neutral

  | Predictor | Estimate | SE | z | p |
  |---|---|---|---|---|
  | Intercept | -0.0718 | 0.0039 | -18.197 | <0.001*** |
  | cond_D | 0.1751 | 0.0032 | 55.358 | <0.001*** |
  | cond_C | 0.2311 | 0.0033 | 70.954 | <0.001*** |
  | pers_rfl | 0.0184 | 0.0018 | 10.106 | <0.001*** |
  | pers_rum | 0.0354 | 0.0018 | 19.183 | <0.001*** |
  | temp_z | 0.0004 | 0.0008 | 0.421 | 0.6735 n.s. |
...
### Regret-word rate (`regret_rate`)
  ...
  | Predictor | Estimate | SE | z | p |
  |---|---|---|---|---|
  | Intercept | 0.1104 | 0.0521 | 2.117 | 0.0342* |
  | cond_D | 0.224 | 0.0418 | 5.364 | <0.001*** |
  | cond_C | 0.2052 | 0.0426 | 4.814 | <0.001*** |
  | pers_rfl | 0.0101 | 0.0242 | 0.419 | 0.6751 n.s. |
  | pers_rum | 0.2562 | 0.0245 | 10.446 | <0.001*** |
  | temp_z | -0.0269 | 0.0111 | -2.424 | 0.0153* |
...
### Counterfactual rate (`cf_rate`)
  ...
  | Predictor | Estimate | SE | z | p |
  |---|---|---|---|---|
  | Intercept | 0.4284 | 0.0999 | 4.289 | <0.001*** |
  | cond_D | 0.2203 | 0.0568 | 3.882 | <0.001*** |
  | cond_C | 0.6723 | 0.0581 | 11.562 | <0.001*** |
  | pers_rfl | 0.0398 | 0.0316 | 1.261 | 0.2074 n.s. |
  | pers_rum | 0.3023 | 0.032 | 9.436 | <0.001*** |
  | temp_z | -0.0601 | 0.0145 | -4.149 | <0.001*** |
...
### Negative emotion rate (`negemo_rate`)
  ...
  | Predictor | Estimate | SE | z | p |
  |---|---|---|---|---|
  | Intercept | 0.0645 | 0.0172 | 3.752 | <0.001*** |
  | cond_D | 0.1155 | 0.019 | 6.074 | <0.001*** |
  | cond_C | 0.0711 | 0.0193 | 3.685 | <0.001*** |
  | pers_rfl | 0.0021 | 0.0131 | 0.156 | 0.8757 n.s. |
  | pers_rum | 0.0143 | 0.0133 | 1.075 | 0.2826 n.s. |
  | temp_z | -0.0118 | 0.006 | -1.976 | 0.0481* |

## Descriptive: Condition means (N=7957)

| Condition | N | Welch D vs N (regret) | d | Welch D vs N (emb_bias) | d |
|---|---|---|---|---|---|
| neutral | 2612 | — | — | — | — |
| deprivation | 2619 | t=17.527, p<0.001 | 0.484 | t=74.324, p<0.001 | 2.055 |
| counterfactual | 2619 | t=9.589, p<0.001 | 0.265 | t=86.791, p<0.001 | 2.4 |
...
| gpt-5-pro | 15 | -0.0026 | -0.0140 | 0.685 |
...
- **H1b (semantic)**: Confirmed — embedding bias significant for both D (z=55.358, p<0.001) and C (z=70.954, p<0.001)
...
CF framing elevates embedding regret bias (beta=0.2311, z=70.954, p<0.001) comparably to deprivation (beta=0.1751, z=55.358, p<0.001). CF rate (deprivation): p=0.0001. This confirms counterfactual framing activates regret-associated semantic representations at the embedding layer.

## Reproducibility
Run: `python3 scripts/run_lme_analysis.py` from project root with .env.real_model sourced.
Full results JSON: results/real_experiments/lme_analysis.json (authoritative, N=7957)
