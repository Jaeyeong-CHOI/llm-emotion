...
- Prompt bank expanded to `v8.9` with **unknown-year query concentration relief / temperature countervoice rotation patch / temperature-entropy circuit-breaker postmortem** scenarios plus new personas (`unknown_year_query_dispersion_auditor_v89`, `temperature_countervoice_rotation_curator_v89`, `temperature_entropy_tripwire_steward_v89`).
- Literature screening quality gate에 **review traceable known-query unknown-year query concentration 가드**(`--max-manual-qc-review-traceable-known-query-unknown-year-top1-query-share`, `--min-manual-qc-review-traceable-known-query-unknown-year-query-coverage`)를 추가해, year 메타데이터가 비어 있는 구간이 소수 query에 잠기는 케이스를 fail-fast로 차단합니다.
- Experiment runner preflight에 **temperature entropy + planned-sample parity guardrail** (`--require-min-temperature-entropy`, `--min-planned-sample-temperature-entropy`, `--max-planned-sample-temperature-top1-share`)를 추가해, temperature count/span이 통과해도 온도축 분산이 무너진 배치를 사전에 차단합니다.
