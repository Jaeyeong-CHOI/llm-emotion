...
- Literature screening quality gate now supports **review query traceability floor** (`--min-manual-qc-review-query-traceability-share`)를 추가해, review 근거가 있어도 source_query가 누락되는 품질 저하를 조기 차단합니다.
- Prompt bank expanded to `v6.5` with **query provenance / multilingual countervoice coverage / live tripwire recovery** scenario·persona additions (`screening_query_provenance_gap_repair`, `prompt_bank_countervoice_translation_hole`, `runner_live_share_tripwire_recovery`, `query_provenance_guard`, `countervoice_translation_curator`, `live_tripwire_controller`). Experiment matrix now includes `screening_prompt_runner_live_traceability_v65`.
- Experiment runner now supports **live run-id attempt share tripwires** (`--max-live-generation-attempt-share-per-run-id`, `--max-live-analysis-attempt-share-per-run-id`, `--max-live-combined-attempt-share-per-run-id`) in addition to post-run pressure reports, so 실행 도중 점유율 과열을 즉시 중단·격리할 수 있습니다.
...
- `docs/`: review protocol, screening rubric, experiment plan, ops notes, reproducibility playbooks (`docs/reproducibility_v65.md`)
