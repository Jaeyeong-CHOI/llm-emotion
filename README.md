...
- Literature screening quality gate now adds **근거 추적성 점검**(`--min-screening-reason-traceability-share`)을 포함해 manual QC 사유 문자열에 핵심 근거 토큰(`include_hits`, `title_hits`, `query_overlap`, `bridge_sentence_hits`)이 충분히 남는지 자동 감시합니다.
- Prompt bank expanded to `v4.8` with evidence-traceback/countervoice-rotation/cell-budget-fairness 시나리오 (`screening_bridge_evidence_traceback`, `prompt_bank_countervoice_pair_rotation`, `runner_attempt_budget_fairness_guard`) and personas (`evidence_traceback_moderator`, `countervoice_rotation_planner`, `cell_budget_fairness_operator`). Experiment matrix now includes `screening_prompt_runner_fairness_v49`.
- Experiment runner now supports **셀 단위 시도 예산 상한**(`--max-attempts-per-cell`)까지 포함해, generation/analysis 재시도 합계가 특정 셀에 과도하게 몰릴 때 조기 차단할 수 있습니다.
...
python3 scripts/check_screening_quality.py --report results/lit_search_report.json --audit results/lit_screening_audit.json --manual-qc-csv results/manual_qc_queue.csv --out results/screening_quality_report.json --out-md results/screening_quality_report.md --run-label screening_qc_v49 --min-balanced-review-rows 6 --min-manual-qc-include-rows 2 --min-manual-qc-review-share 0.25 --max-manual-qc-review-share 0.85 --max-manual-qc-label-dominance 0.75 --min-screening-reason-diversity 6 --max-top-screening-reason-share 0.65 --min-screening-reason-entropy 0.55 --min-screening-reason-traceability-share 0.65 --min-manual-qc-query-entropy 0.50 --min-manual-qc-risk-reason-entropy 0.45 --min-manual-qc-review-reason-entropy 0.35 --min-manual-qc-source-groups 3 --max-manual-qc-single-query-share 0.45 --max-manual-qc-unknown-query-share 0.20 --max-empty-screening-reason-share 0.10
python3 scripts/build_evidence_table.py --in refs/openalex_results.jsonl --out docs/evidence-table.md

python3 scripts/run_experiments.py --config ops/experiment_matrix.json --run-label smoke_v49_plan --plan-only --print-selection --selection-report results/selection_report_smoke_v49.json --selection-csv results/selection_report_smoke_v49.csv --require-min-scenarios 4 --require-min-personas 4 --require-min-unique-scenario-tags 4 --require-min-unique-persona-style-tags 7 --require-min-temperature-count 2 --require-min-temperature-span 0.3 --require-min-repeats 1 --require-min-condition-cells 48 --require-min-run-cells 20 --require-min-run-ids 10 --require-min-total-samples 13000 --require-min-planned-samples-per-run 1100 --require-min-selected-scenarios 34 --require-min-selected-personas 27 --require-min-selected-scenario-tags 20 --require-min-selected-persona-style-tags 22 --require-prompt-bank-version v4.8 --require-freeze-artifact refs/openalex_results.jsonl --require-freeze-artifact results/lit_search_report.json --require-freeze-artifact results/screening_quality_report.json --manifest-note-file docs/experiment-plan.md --manifest-note "preflight v49"
printf '%s\n' screening_prompt_replay_v36 screening_prompt_runner_hardening_v37 screening_prompt_runner_resilience_v38 screening_prompt_runner_autonomy_v39 screening_prompt_runner_reliability_v40 screening_prompt_runner_stagewise_v41 screening_prompt_runner_timeout_v42 screening_prompt_runner_multilingual_v44 screening_prompt_runner_quarantine_v45 screening_prompt_runner_reentry_v46 screening_prompt_runner_budget_v47 screening_prompt_runner_traceability_v48 screening_prompt_runner_fairness_v49 > /tmp/llm_emotion_run_ids_v49.txt
printf '%s\n' screening_prompt_runner_fairness_v49 > /tmp/llm_emotion_run_ids_v49_exec.txt
python3 scripts/run_experiments.py --config ops/experiment_matrix.json --run-label smoke_v49_exec --run-id-file /tmp/llm_emotion_run_ids_v49_exec.txt --fail-on-missing-run-id --manifest-markdown --max-runs 1 --max-retries 1 --max-generation-retries 2 --max-analysis-retries 0 --max-generation-attempts-per-run-id 3 --max-analysis-attempts-per-run-id 2 --max-attempts-per-cell 4 --require-min-generation-success-rate 0.8 --require-min-analysis-success-rate 0.8 --retry-backoff-seconds 1 --generation-timeout-seconds 120 --analysis-timeout-seconds 90 --max-run-seconds 180 --continue-on-error --max-failed-cells 1 --max-failure-rate 0.5 --max-failure-streak 1 --require-min-successful-cells 1 --require-min-success-rate 0.4 --require-min-run-id-success-rate 0.4 --execution-log-jsonl results/experiments/smoke_v49_exec/command_log.jsonl --quarantine-json results/experiments/smoke_v49_exec/quarantine_candidates.json --quarantine-csv results/experiments/smoke_v49_exec/quarantine_candidates.csv --require-min-total-samples 1000
```
