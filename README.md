...
- Prompt bank expanded to `v6.9` with **review query-counterexample cluster sweep / countervoice ladder audit / live stage-gap recovery** additions (`screening_query_cluster_counterexample_sweep`, `prompt_bank_countervoice_ladder_audit`, `runner_live_stage_gap_recovery_drill`, `query_cluster_diversity_guard`, `countervoice_ladder_calibrator`, `live_stage_gap_sentinel`). Experiment matrix now includes `screening_prompt_runner_live_stage_gap_v69`.
- Experiment runner now supports **live stage attempt-share gap tripwire** (`--max-live-stage-attempt-share-gap-per-run-id`, `--max-live-retry-over-selection-ratio-per-run-id`) in addition to stage retry governance and `budget_report.json/md` + `--budget-violations-json`, making live stage-imbalance aborts reproducible and easier to audit.
