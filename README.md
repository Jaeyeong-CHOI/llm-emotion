...
- Prompt bank expanded to `v3.8` with recall-focused and runner-safety scenarios (`screening_keyword_semantic_shadow`, `prompt_bank_recall_counterweight_injection`, `runner_preflight_success_floor_tripwire`) plus dedicated personas (`retrieval_recall_mapper`, `success_floor_operator`). Experiment matrix now includes `screening_prompt_runner_autonomy_v39`.
- Experiment runner preflight now records aggregate unique scenario/persona/tag coverage across the selected batch and can enforce those floors with `--require-min-selected-scenarios`, `--require-min-selected-personas`, `--require-min-selected-scenario-tags`, and `--require-min-selected-persona-style-tags`. Execution mode also supports a failure-streak circuit breaker via `--max-failure-streak`, and batch-level minimum successful-cell guardrail via `--require-min-successful-cells`.
...
python3 scripts/check_screening_quality.py --report results/lit_search_report.json --audit results/lit_screening_audit.json --manual-qc-csv results/manual_qc_queue.csv --out results/screening_quality_report.json --out-md results/screening_quality_report.md --run-label screening_qc_v37 --min-balanced-review-rows 6
python3 scripts/build_evidence_table.py --in refs/openalex_results.jsonl --out docs/evidence-table.md

python3 scripts/run_experiments.py --config ops/experiment_matrix.json --run-label smoke_v38_plan --plan-only --print-selection --selection-report results/selection_report_smoke_v38.json --selection-csv results/selection_report_smoke_v38.csv --require-min-scenarios 4 --require-min-personas 4 --require-min-unique-scenario-tags 4 --require-min-unique-persona-style-tags 7 --require-min-temperature-count 2 --require-min-temperature-span 0.3 --require-min-repeats 1 --require-min-condition-cells 48 --require-min-run-cells 16 --require-min-run-ids 8 --require-min-total-samples 11200 --require-min-planned-samples-per-run 1100 --require-min-selected-scenarios 33 --require-min-selected-personas 27 --require-min-selected-scenario-tags 20 --require-min-selected-persona-style-tags 30 --require-prompt-bank-version v3.8 --require-freeze-artifact refs/openalex_results.jsonl --require-freeze-artifact results/lit_search_report.json --require-freeze-artifact results/screening_quality_report.json --manifest-note-file docs/experiment-plan.md --manifest-note "preflight v38"
printf '%s\n' screening_precision_rebalance_v35 prompt_runner_resilience_v35 screening_prompt_replay_v36 screening_prompt_runner_hardening_v37 screening_prompt_runner_resilience_v38 screening_prompt_runner_autonomy_v39 > /tmp/llm_emotion_run_ids_v38.txt
python3 scripts/run_experiments.py --config ops/experiment_matrix.json --run-label smoke_v38_exec --run-id-file /tmp/llm_emotion_run_ids_v38.txt --fail-on-missing-run-id --manifest-markdown --max-runs 1 --max-retries 1 --retry-backoff-seconds 1 --continue-on-error --max-failed-cells 1 --max-failure-rate 0.5 --max-failure-streak 1 --require-min-successful-cells 1 --execution-log-jsonl results/experiments/smoke_v38_exec/command_log.jsonl --require-min-total-samples 1000
```
