...
- Literature screening quality gate now adds **include/review 라벨별 traceability floor**(`--min-include-reason-traceability-share`, `--min-review-reason-traceability-share`)까지 포함해 전체 평균에 가려지는 핵심 screening handoff 품질 저하를 별도로 감시합니다.
- Prompt bank expanded to `v5.1` with review-traceability/countervoice-replay/attempt-pressure 시나리오 (`screening_review_traceability_rescue`, `prompt_bank_countervoice_gap_replay`, `runner_attempt_pressure_ratio_tripwire`) and personas (`review_traceability_arbiter`, `attempt_pressure_balancer`). Experiment matrix now includes `screening_prompt_runner_attempt_pressure_v51`.
- Experiment runner now emits **attempt pressure ratio**(선택 셀 점유율 대비 시도 점유율) in `budget_report.json` / `budget_report.md` and supports `--max-attempt-over-selection-ratio` to fail fast when one run id burns retries disproportionately to its planned coverage.
