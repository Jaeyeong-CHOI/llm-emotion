...
- Literature screening quality now includes a reproducible gate-check step: `scripts/check_screening_quality.py` reads `lit_search_report.json`/`lit_screening_audit.json`/`manual_qc_queue.csv` and emits JSON/Markdown quality reports with threshold results, hotspot reasons, and query-drift suggestions.
- Prompt bank expanded to `v3.3` with multilingual/calibration scenarios (`cross_lingual_annotation_drift`, `fine_grained_emotion_boundary_case`, `manual_qc_escalation_handoff`, `batch_resume_after_partial_failure`) and four new personas for cross-lingual calibration, manual-QC handoff, and partial-failure recovery.
- Experiment runner reproducibility now supports batch run selection and retry logging: `--run-id-file`, `--max-retries`, `--retry-backoff-seconds`, and `--execution-log-jsonl` add replayable batch control plus per-attempt JSONL logs alongside existing resume hash verification.
...
python3 scripts/check_screening_quality.py --report results/lit_search_report.json --audit results/lit_screening_audit.json --manual-qc-csv results/manual_qc_queue.csv --out results/screening_quality_report.json --out-md results/screening_quality_report.md --run-label screening_qc_v33
