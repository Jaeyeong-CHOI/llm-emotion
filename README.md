...
- Prompt bank expanded to `v5.4` with screening/prompt/runner 연계 시나리오 (`screening_review_bridge_counterexample_recheck`, `prompt_bank_countervoice_holdout_rotation`, `runner_runid_failure_streak_guard`) and personas (`review_bridge_traceability_keeper`, `runid_failure_streak_marshal`). Experiment matrix now includes `screening_prompt_runner_streak_v54`.
...
- `docs/`: review protocol, screening rubric, experiment plan, ops notes, reproducibility playbooks (`docs/reproducibility_v54.md`)
