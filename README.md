...
- Prompt bank expanded to `v6.7` with **review query concentration / countervoice domain parity / run-id stage-imbalance tripwire** additions (`screening_review_query_concentration_relief`, `prompt_bank_countervoice_domain_parity`, `runner_runid_stage_imbalance_tripwire`, `review_query_diversity_curator`, `countervoice_domain_parity_designer`, `stage_imbalance_tripwire_operator`). Experiment matrix now includes `screening_prompt_runner_stage_imbalance_v67`.
- Experiment runner now supports **budget violation export** (`--budget-violations-json`) in addition to `budget_report.json/md`, making threshold breaches reproducible and easier to audit.
