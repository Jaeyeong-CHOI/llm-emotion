...
- Prompt bank expanded to `v6.0` with screening/prompt/runner 연계 시나리오 (`screening_crossquery_consensus_repair`, `prompt_bank_countervoice_temperature_ladder`, `runner_live_stage_gap_tripwire`) and personas (`crossquery_consensus_moderator`, `live_budget_tripwire_operator`). Experiment matrix now includes `screening_prompt_runner_live_gap_v60`.
- Experiment runner now supports **stage total attempt gap tripwire** (`--max-stage-total-attempt-gap-share`) in addition to run-id별 gap guard, so 생성/분석 전체 시도량 불균형도 배치 종료 조건으로 통제할 수 있습니다.
