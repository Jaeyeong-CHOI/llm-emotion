...
- Literature screening quality gate now supports **dedup 안정성 게이트** (`--max-manual-qc-dedup-label-conflict-share`, `--max-manual-qc-dedup-score-range-alert-share`)를 추가해, 수동 QC 큐에서 중복 병합 라벨 충돌과 점수 변동 폭이 누적되는 상황을 조기에 차단합니다.
- Prompt bank expanded to `v6.4` with **dedup conflict / scenario-label coverage / preflight guardrail** scenario/persona additions (`screening_dedup_conflict_triage`, `prompt_bank_label_coverage_rebalance`, `runner_scenario_label_guardrail`, `dedup_conflict_auditor`, `label_coverage_rebalancer`, `scenario_label_guard_operator`). Experiment matrix now includes `screening_dedup_label_guard_v64`.
- Experiment runner now supports **scenario label coverage guardrails** (`--require-min-unique-scenario-labels`, `--require-min-selected-scenario-labels`) in addition to tag/style coverage floors, so 태그 수는 충분하지만 실제 scenario label 축이 편향된 배치를 preflight에서 fail-fast로 막을 수 있습니다.
...
- `docs/`: review protocol, screening rubric, experiment plan, ops notes, reproducibility playbooks (`docs/reproducibility_v64.md`)
