...
- Prompt bank expanded to `v5.6` with screening/prompt/runner 연계 시나리오 (`screening_review_evidence_link_decay`, `prompt_bank_countervoice_parity_gap`, `runner_stage_failure_streak_tripwire`) and personas (`review_traceability_maintainer`, `stage_failure_streak_monitor`). Experiment matrix now includes `screening_prompt_runner_stage_streak_v56`.
- Experiment runner now supports stage-specific streak tripwires (`--max-generation-failure-streak`, `--max-analysis-failure-streak`) so 생성/분석 단계 연속 실패를 분리 감시하고 조기 중단 기준을 더 세밀하게 운영할 수 있습니다.
