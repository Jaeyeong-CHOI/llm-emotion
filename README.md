...
- Literature screening quality gate now adds **review confidence 분산 점검**(`--min-manual-qc-review-confidence-bins`, `--min-manual-qc-review-confidence-entropy`) to detect review 큐가 단일 confidence band로 붕괴되는 early warning을 제공합니다.
- Prompt bank expanded to `v4.7` with run-id 예산 고갈/confidence-band 붕괴/counterpair 다양성 저하 시나리오 (`runner_per_run_budget_exhaustion`, `screening_confidence_band_overcollapse`, `prompt_bank_counterpair_entropy_drop`) and personas (`runid_budget_sentinel`, `confidence_band_calibrator`, `counterpair_entropy_curator`). Experiment matrix now includes `screening_prompt_runner_budget_v47` + `screening_prompt_runner_traceability_v48`.
- Experiment runner now supports **run-id별 재시도 예산 상한**(`--max-generation-attempts-per-run-id`, `--max-analysis-attempts-per-run-id`)과 **셀 wall-clock 상한**(`--max-run-seconds`)으로 특정 run-id 독점/장시간 교착을 fail-fast로 차단할 수 있습니다.
