...
- Experiment runner now supports **scenario label dominance guardrail** (`--require-max-scenario-label-dominance`) alongside live stage-governance tripwires, so preflight can fail-fast when selected scenarios are overly concentrated on one label.
