...
- Literature screening quality gate now supports **라벨별 bridge-signal 분리 점검**(`--min-include-bridge-signal-share`, `--min-review-bridge-signal-share`)을 추가해 review handoff에서 근거 문장 연결이 실제로 남는지 별도로 추적할 수 있습니다.
- Prompt bank expanded to `v5.2` with confidence-band/countervoice-entropy/failure-pressure 시나리오 (`screening_review_confidence_band_recovery`, `prompt_bank_countervoice_entropy_backfill`, `runner_failure_pressure_tripwire`) and personas (`review_band_calibration_lead`, `failure_pressure_governor`). Experiment matrix now includes `screening_prompt_runner_failure_pressure_v52`.
- Experiment runner budget report now emits **failure pressure ratio**(실패 셀 점유율 대비 선택 셀 점유율) and supports `--max-failure-over-selection-ratio` in addition to attempt pressure control, so a single run id's failure concentration can be fail-fast gated.
