...
- Prompt bank expanded to `v6.1` with scenario/persona additions (`screening_evidence_conflict_graph_repair`, `prompt_bank_countervoice_sparse_cluster_patch`, `runner_stage_budget_quota_guard`, `evidence_conflict_resolver`, `stage_quota_controller`). Experiment matrix now includes `screening_prompt_runner_quota_v61`.
- Experiment runner now supports **stage attempt imbalance ratio guard** (`--max-stage-attempt-imbalance-ratio-per-run-id`, `--max-stage-total-attempt-imbalance-ratio`) in addition to share-gap tripwires, so 생성/분석 단계의 비대칭 시도 압박을 비율 관점에서도 fail-fast 제어할 수 있습니다.
...
- `docs/`: review protocol, screening rubric, experiment plan, ops notes, reproducibility playbooks (`docs/reproducibility_v60.md`)
