...
- Prompt bank expanded to `v6.8` with **review query-cluster floor / countervoice intensity band / stage retry-mix guard** additions (`screening_review_query_cluster_floor`, `prompt_bank_countervoice_intensity_band`, `runner_stage_retry_mix_guard`, `review_query_cluster_guard`, `countervoice_intensity_curator`, `stage_retry_mix_operator`). Experiment matrix now includes `screening_prompt_runner_retry_mix_v68`.
- Experiment runner now supports **stage-specific retry share governance** (`--max-generation-retry-share-per-run-id`, `--max-analysis-retry-share-per-run-id`) in addition to `budget_report.json/md` + `--budget-violations-json`, making retry concentration breaches reproducible and easier to audit.
...
- `docs/`: review protocol, screening rubric, experiment plan, ops notes, reproducibility playbooks (`docs/reproducibility_v68.md`)
