...
- Prompt bank expanded to `v6.6` with **review-query entropy / countervoice parity / live failed-share tripwire** additions (`screening_review_query_entropy_guard`, `prompt_bank_countervoice_strength_parity`, `runner_live_failed_share_tripwire`, `review_query_entropy_guardian`, `countervoice_parity_calibrator`, `live_failure_share_operator`). Experiment matrix now includes `screening_prompt_runner_live_failed_share_v66`.
- Experiment runner now supports **preflight markdown export** (`--preflight-markdown`) alongside `preflight.json/csv`, making gate/coverage checks easier to review and hand off.
