...
- Literature screening quality gate now supports **review bridge-traceability 결합 점검**(`--min-review-bridge-traceability-share`)까지 확장되어, review handoff에서 bridge signal과 근거 traceability가 함께 유지되는지 추적할 수 있습니다.
- Prompt bank expanded to `v5.3` with bridge-traceability/counterexample-rotation/failed-share 시나리오 (`screening_review_bridge_traceability_coupling`, `prompt_bank_counterexample_rotation_gap`, `runner_failed_cell_share_tripwire`) and persona (`failure_share_quarantine_steward`). Experiment matrix now includes `screening_prompt_runner_failed_share_v53`.
- Experiment runner budget guard now supports `--max-failed-cell-share-per-run-id` in addition to attempt/failure-over-selection ratio controls, so one run id monopolizing failed cells can be fail-fast gated.
