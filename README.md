...
- Experiment runner now supports **live stage attempt-share gap tripwire** (`--max-live-stage-attempt-share-gap-per-run-id`) in addition to stage retry governance and `budget_report.json/md` + `--budget-violations-json`, making live stage-imbalance aborts reproducible and easier to audit.
