...
- Prompt bank is now `v4.7`, adding traceability/countervoice/attempt-log scenarios (`screening_reason_traceability_gap`, `prompt_bank_countervoice_dropout`, `runner_attempt_log_granularity_debt`) on top of prior recall/drift/recovery sets.
- Persona bank now also includes `reason_traceability_auditor`, `countervoice_balance_keeper`, and `attempt_log_forensics_operator` to probe 스크리닝 근거 추적성, 반대축 프롬프트 균형, 시도 로그 포렌식 기반 운영 전략.
- Scenario rows carry `tags` and stable `id`s for reproducible focused subsets (`scenario_tags` and `scenario_ids`).
- Experiment matrix now includes `screening_prompt_runner_traceability_v48`, while runner supports aggregate batch-coverage gates (`--require-min-selected-scenarios`, `--require-min-selected-personas`, `--require-min-selected-scenario-tags`, `--require-min-selected-persona-style-tags`) plus `--continue-on-error`, `--max-failed-cells`, `--max-failure-streak`, `--max-failed-cells-per-run-id`, `--require-min-successful-cells`, `--require-min-success-rate`, `--require-min-run-id-success-rate`, stage-specific retry controls (`--max-generation-retries`, `--max-analysis-retries`), stage timeout controls (`--generation-timeout-seconds`, `--analysis-timeout-seconds`), per-cell wall-clock cap (`--max-run-seconds`), stage success floors (`--require-min-generation-success-rate`, `--require-min-analysis-success-rate`), JSONL command logs, and `--resume-verify-hashes`.
