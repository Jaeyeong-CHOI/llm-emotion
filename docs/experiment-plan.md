...
- Prompt bank is now `v5.3`, adding review-bridge-traceability/counterexample-rotation/failed-share scenarios (`screening_review_bridge_traceability_coupling`, `prompt_bank_counterexample_rotation_gap`, `runner_failed_cell_share_tripwire`) on top of prior confidence-band/failure-pressure sets.
- Persona bank now also includes `failure_share_quarantine_steward` (alongside `review_band_calibration_lead`, `failure_pressure_governor`) to probe run-id 실패 과점 격리 전략.
