...
- Prompt bank is now `v5.0`, adding semantic-bridge/countervoice-coverage/budget-pressure scenarios (`screening_alias_semantic_bridge_gap`, `prompt_bank_countervoice_coverage_debt`, `runner_budget_pressure_handoff`) on top of prior traceability/fairness sets.
- Persona bank now also includes `semantic_bridge_triager`, `countervoice_coverage_archivist`, and `budget_pressure_dispatcher` to probe alias-bridge recall, 반대관점 커버리지 부채, run-id별 예산 압박 handoff 전략.
- Scenario rows carry `tags` and stable `id`s for reproducible focused subsets (`scenario_tags` and `scenario_ids`).
- Experiment matrix now includes `screening_prompt_runner_budget_pressure_v50`, while runner supports aggregate batch-coverage gates (`--require-min-selected-scenarios`, `--require-min-selected-personas`, `--require-min-selected-scenario-tags`, `--require-min-selected-persona-style-tags`) plus `--continue-on-error`, `--max-failed-cells`, `--max-failure-streak`, `--max-failed-cells-per-run-id`, `--require-min-successful-cells`, `--require-min-success-rate`, `--require-min-run-id-success-rate`, stage-specific retry controls (`--max-generation-retries`, `--max-analysis-retries`), stage timeout controls (`--generation-timeout-seconds`, `--analysis-timeout-seconds`), per-cell wall-clock cap (`--max-run-seconds`), stage success floors (`--require-min-generation-success-rate`, `--require-min-analysis-success-rate`), JSONL command logs, `--resume-verify-hashes`, and fairness ceilings/reporting (`--max-selected-cell-share-per-run-id`, `--max-attempt-share-per-run-id`, `budget_report.json`, `budget_report.md`).
...
- Use `--max-selected-cell-share-per-run-id` and `--max-attempt-share-per-run-id` to prevent one run id from monopolizing batch slots or retries; review `budget_report.json` / `budget_report.md` after every smoke execution
