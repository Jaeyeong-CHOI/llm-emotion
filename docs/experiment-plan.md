...
- Prompt bank is now `v4.4`, adding precision/recall pivot debt, antithesis rotation debt, and stage-quarantine failover scenarios (`screening_precision_recall_pivot_debt`, `prompt_bank_antithesis_rotation_debt`, `runner_stage_quarantine_failover`) on top of prior recall/drift/recovery sets.
- Persona bank now also includes `screening_precision_recall_negotiator`, `antithesis_rotation_curator`, and `stage_quarantine_supervisor` to probe 스크리닝 정밀도-재현율 협상, 반대축 프롬프트 교체 관리, 단계 실패 격리 운영 전략.
- Scenario rows carry `tags` and stable `id`s for reproducible focused subsets (`scenario_tags` and `scenario_ids`).
- Experiment matrix now includes `screening_prompt_runner_quarantine_v45`, while runner supports aggregate batch-coverage gates (`--require-min-selected-scenarios`, `--require-min-selected-personas`, `--require-min-selected-scenario-tags`, `--require-min-selected-persona-style-tags`) plus `--continue-on-error`, `--max-failed-cells`, `--max-failure-streak`, `--max-failed-cells-per-run-id`, `--require-min-successful-cells`, `--require-min-success-rate`, `--require-min-run-id-success-rate`, stage-specific retry controls (`--max-generation-retries`, `--max-analysis-retries`), stage timeout controls (`--generation-timeout-seconds`, `--analysis-timeout-seconds`), stage success floors (`--require-min-generation-success-rate`, `--require-min-analysis-success-rate`), JSONL command logs, and `--resume-verify-hashes`.
