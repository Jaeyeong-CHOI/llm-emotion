...
python3 scripts/run_experiments.py \
  ...
  --run-label repro_v55_preflight \
  --plan-only \
  --include-run-id screening_prompt_runner_counterexample_v55 \
...
python3 scripts/run_experiments.py \
  --config ops/experiment_matrix.json \
  --run-label repro_v55_smoke \
  --run-id-file /tmp/llm_emotion_run_ids_v55_exec.txt \
  --fail-on-missing-run-id \
  --max-runs 1 \
  --max-retries 1 \
  --max-failed-cells 1 \
