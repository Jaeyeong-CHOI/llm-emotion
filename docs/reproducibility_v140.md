...
2. **프롬프트 뱅크 확장**
   - `prompts/prompt_bank_ko.json` 버전을 `v139.0`에서 `v140.0`으로 상향.
   - 시나리오 3개(`screening_unknown_year_group_top27_entropy_stability_v140`, `prompt_bank_countervoice_protocol_trace_mesh_v140`, `runner_temperature_p99_p25_reprolock_tripwire_v140`) 추가.
   - 페르소나 3개(`unknown_year_group_top27_entropy_steward_v140`, `protocol_trace_countervoice_curator_v140`, `p99_p25_reprolock_operator_v140`) 추가.
   - `ops/experiment_matrix.json`에 `screening_prompt_runner_unknown_year_group_top27_temperature_p99_p25_v140` 러너 스모크 ID 추가.
3. **실험 러너 고도화**
...
python3 scripts/run_experiments.py \
  ...
  --run-label smoke_v140_plan \
  --plan-only \
  --include-run-id screening_prompt_runner_unknown_year_group_top27_temperature_p99_p25_v140 \
  --fail-on-missing-run-id \
  --selection-report results/selection_report_smoke_v140.json \
  --selection-csv results/selection_report_smoke_v140.csv \
  --repro-lock-json results/selection_report_smoke_v140.lock.json \
  --preflight-markdown \
  --require-prompt-bank-version v140.0 \
  ...
  --manifest-note "preflight v140 top27 entropy stability + p99/p25 repro-lock guard"
```
