...
python3 scripts/check_screening_quality.py \
  ...
  --run-label screening_qc_v54 \
  --min-review-bridge-traceability-given-bridge-share 0.70
```

핵심: `review_bridge_traceability_given_bridge_share`가 bridge-signal subset에서 최소 기준을 만족하는지 확인.
...
assert bank['version']=='v5.4', bank['version']
...
python3 scripts/run_experiments.py \
  ...
  --run-label repro_v54_preflight \
  --plan-only \
  --include-run-id screening_prompt_runner_streak_v54 \
  --fail-on-missing-run-id \
  --require-prompt-bank-version v5.4 \
...
python3 scripts/run_experiments.py \
  --config ops/experiment_matrix.json \
  --run-label repro_v54_smoke \
  --run-id-file /tmp/llm_emotion_run_ids_v54_exec.txt \
  --fail-on-missing-run-id \
  --max-runs 1 \
  --max-retries 1 \
  --max-failed-cells 1 \
