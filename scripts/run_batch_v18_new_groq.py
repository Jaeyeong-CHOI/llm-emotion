#!/usr/bin/env python3
"""
Batch v18: New Groq models — Kimi-K2-0905 + Allam-2-7B
Goals:
  - moonshotai/kimi-k2-instruct-0905: Updated Kimi-K2 variant (compare vs existing kimi-k2-instruct)
  - allam-2-7b: Arabic-focused LLM (cross-linguistic family extension)
  - 3 conditions × 3 personas × 6 prompts × 2 temps → ~216 samples/model (~432 total)

Output: results/real_experiments/batch_v18_new_groq.jsonl
"""
...
OUT_FILE = ROOT / "results" / "real_experiments" / "batch_v18_new_groq.jsonl"
                ...
                for temp in [0.7, 0.9]:
                    for prompt in prompts:
                        sample_id = str(uuid.uuid5(
                            uuid.NAMESPACE_DNS,
                            f"v18:{model}:{condition}:{persona_name}:{temp}:{prompt[:40]}"
                        ))
                        ...
                        try:
                            ...
                            row = {
                                ...
                                "temperature": temp,
                                "scenario_id": f"v18_{condition}_{prompts.index(prompt):02d}",
                                "prompt": prompt,
                                "output": text,
                                "ts": time.time(),
                            }
