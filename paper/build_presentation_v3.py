#!/usr/bin/env python3
"""Build llm-emotion-presentation-v3.pptx — 15-slide professional research deck.
Updated: N=6709, 32 models, 7 families, latest LME/Welch stats."""
...
def slide_chrome(slide, page_num, total=15, dark_bg=False):
...
add_stat_card(s1, Inches(8.5), Inches(4.5), Inches(1.8), Inches(1.1),
              "6,709", "Total Samples", C_ACCENT, RGBColor(35, 42, 60))
add_stat_card(s1, Inches(10.5), Inches(4.5), Inches(1.8), Inches(1.1),
              "32", "Model Variants", C_BLUE, RGBColor(35, 42, 60))
...
agenda_items = [
    ...
    ("02", "Experimental Design", "3 conditions \u00d7 3 personas \u00d7 continuous temperature across 32 model variants"),
    ("03", "Results: Condition Effects", "LME analysis of prompt framing (N=6,709)"),
    ...
    ("06", "Results: Cross-Model Replication", "Effect sizes across 7 model families"),
...
cards = [
    ("3 \u00d7 3 \u00d7 T", "Factorial Design", C_ACCENT),
    ("N = 6,709", "Total Samples", C_BLUE),
    ("32 Variants", "7 Model Families", C_GREEN),
...
# Note — updated temperature and scenario info
add_multiline_textbox(s4, Inches(0.8), Inches(5.7), Inches(11.5), Inches(1.2), [
    ("Temperature as continuous covariate (multiple values: 0.2\u20131.0) across 32 model variants.", 11, False, C_GRAY1, PP_ALIGN.LEFT),
    ("Models span GPT, Claude, Gemini, and open-weight families (7 families).", 11, False, C_GRAY1, PP_ALIGN.LEFT),
    ("Scenario bank: neutral = 17, deprivation = 20, counterfactual = 14 scenarios.  |  N/D/C: 2,225 / 2,234 / 2,250 samples.", 11, False, C_GRAY1, PP_ALIGN.LEFT),
])
...
# Simplified horizontal bar chart — updated betas
conditions = [
    ("Neutral (N)", 0.0, C_GRAY1, "baseline (\u2212 0.034 \u00b1 0.075)"),
    ("Deprivation (D)", 0.181, C_ACCENT, "\u03b2=0.181, z=43.0*** (+0.113 \u00b1 0.076)"),
    ("Counterfactual (C)", 0.233, C_BLUE, "\u03b2=0.233, z=52.8*** (+0.133 \u00b1 0.072)"),
]
...
for i, (label, val, color, stat) in enumerate(conditions):
    ...
    bar_w = max(Inches(0.3), int(max_bar_w * (val / 0.30))) if val > 0 else Inches(0.3)
...
# Right side: LME table — updated values
add_textbox(s5, Inches(7.2), Inches(1.2), Inches(5.5), Inches(0.4),
            "LME Results (N = 6,709)", font_size=14, bold=True, color=C_DARK)

lme_rows = [
    ["Predictor", "Outcome", "\u03b2", "z", "p"],
    ["cond_D", "Emb bias", "0.181", "43.0", "<.001"],
    ["cond_C", "Emb bias", "0.233", "52.8", "<.001"],
    ["Ruminative", "Emb bias", "0.038", "19.5", "<.001"],
    ["Reflective", "Emb bias", "0.019", "9.9", "<.001"],
    ["Temperature", "Emb bias", "\u22120.001", "\u22121.4", ".154 n.s."],
]
...
add_highlight_box(s5, Inches(0.8), Inches(5.0), Inches(11.5), Inches(1.2),
                  ...
                  "Temperature shows no significant effect (\u03b2=\u22120.001, z=\u22121.4, p=.154), confirming the effect is "
                  "prompt-driven, not temperature-driven.",
...
emb_items = [
    "D vs N: d = 1.943 (large effect), t = 64.9",
    "C vs N: d = 2.279 (large effect), t = 76.2",
    "Both conditions significantly activate regret-associated semantic space",
    "LME confirms: \u03b2_D = 0.181***, \u03b2_C = 0.233***",
]
...
# Persona comparison table — updated
persona_rows = [
    ["Persona", "Emb Bias \u03b2", "Emb Bias z", "Interpretation"],
    ["None (baseline)", "\u2014", "\u2014", "Reference level"],
    ["Reflective", "0.019", "9.9***", "Controlled self-reflection"],
    ["Ruminative", "0.038", "19.5***", "Strongest activation lever"],
]
...
add_stat_card(s7, Inches(0.8), Inches(4.8), Inches(3.5), Inches(1.3),
              "z = 19.5", "Ruminative Emb Bias (p < .001)", C_PURPLE)
add_stat_card(s7, Inches(4.8), Inches(4.8), Inches(3.5), Inches(1.3),
              "z = 9.9", "Reflective Emb Bias (p < .001)", C_PURPLE)
...
add_textbox(s8, Inches(0.8), Inches(1.2), Inches(11), Inches(0.4),
            "Welch t-test Effect Sizes: d(D vs N) Embedding Bias per Model  |  32 variants, 7 families",
            font_size=14, bold=True, color=C_DARK)

# Horizontal bar chart — per-model d values (representative subset)
models_data = [
    ...
    ("Average (32 models)", 1.943, C_DARK),
...
for i, (model, d_val, color) in enumerate(models_data):
    ...
    add_textbox(s8, Inches(0.8), y, Inches(2.6), Inches(0.35),
                ...
                bold=(model == "Average (32 models)"))
    bar_w = int(max_w * (d_val / 3.5))
    is_avg = model == "Average (32 models)"
    ...
    add_textbox(s8, chart_left + bar_w + Inches(0.1), y, Inches(0.8), Inches(0.35),
                f"d={d_val:.2f}" if not is_avg else f"d={d_val:.3f}", font_size=9, bold=is_avg, color=C_DARK)

# Key finding
add_highlight_box(s8, Inches(0.8), Inches(6.6), Inches(11.5), Inches(0.5),
                  "All 7 model families show directional replication. Overall: d=1.943 (D vs N), d=2.279 (C vs N). Range: 0.42\u20133.37.",
...
limitations = [
    ...
    ("d-value inflation risk", "Large Cohen\u2019s d partly reflects low within-condition variance; interpret with LME \u03b2 jointly."),
]
...
future = [
    ...
    "Model random-effect analysis (variance decomposition)",
]
...
# 4 finding cards in 2x2 grid — updated stats
findings = [
    ...
    ("3", "Persona > Prompt", "Ruminative persona instruction is the strongest predictor \u2014 more reliable than user-prompt framing for affect-laden generation.",
     "\u03b2=0.038, z=19.5 (embedding)", C_PURPLE),
    ("4", "Cross-Model Replication", "Effects replicate directionally across all 32 model variants tested, spanning 7 families.",
...
add_textbox(sA1, Inches(0.8), Inches(0.3), Inches(10), Inches(0.6),
            "Appendix A1: Full LME Results (N = 6,709)", font_size=24, bold=True, color=C_DARK)
add_rect(sA1, Inches(0.8), Inches(0.85), Inches(1.5), Inches(0.04), fill=C_ACCENT)

lme_full_rows = [
    ["Outcome", "Predictor", "\u03b2", "z", "p", "Sig"],
    ["Emb bias", "cond_D", "0.181", "43.0", "<.001", "***"],
    ["Emb bias", "cond_C", "0.233", "52.8", "<.001", "***"],
    ["Emb bias", "persona_rum", "0.038", "19.5", "<.001", "***"],
    ["Emb bias", "persona_refl", "0.019", "9.9", "<.001", "***"],
    ["Emb bias", "temperature", "\u22120.001", "\u22121.4", ".154", "n.s."],
...
welch_rows = [
    ["Contrast", "Outcome", "t", "Cohen\u2019s d", "Direction", "N"],
    ["D vs N", "Emb bias", "64.9", "1.943", "D > N", "6,709"],
    ["C vs N", "Emb bias", "76.2", "2.279", "C > N", "6,709"],
    ["D vs N (GPT-3.5)", "Emb bias", "\u2014", "3.37", "D > N", "~240"],
    ["D vs N (GPT-5.4-mini)", "Emb bias", "\u2014", "0.42", "D > N", "~240"],
    ["C vs D", "Emb bias", "\u2014", "0.27", "C > D", "6,709"],
]
add_table(sA2, Inches(0.8), Inches(1.2), Inches(11.5), welch_rows,
          [Inches(2.5), Inches(1.5), Inches(1.2), Inches(1.5), Inches(1.5), Inches(1.5)],
          header_color=C_BLUE, font_size=12)

# Descriptive stats — condition means
add_textbox(sA2, Inches(0.8), Inches(3.7), Inches(11), Inches(0.4),
            "Condition Means (Embedding Bias)", font_size=14, bold=True, color=C_DARK)

mean_rows = [
    ["Condition", "Mean", "SD", "N"],
    ["Neutral (N)", "\u22120.034", "0.075", "2,225"],
    ["Deprivation (D)", "+0.113", "0.076", "2,234"],
    ["Counterfactual (C)", "+0.133", "0.072", "2,250"],
]
add_table(sA2, Inches(0.8), Inches(4.1), Inches(6.0), mean_rows,
          [Inches(2.0), Inches(1.3), Inches(1.3), Inches(1.3)],
          header_color=C_TEAL, font_size=12)

# Stat cards — updated
add_stat_card(sA2, Inches(7.5), Inches(3.8), Inches(2.0), Inches(1.2),
              "d = 1.943", "D vs N (Overall)", C_ACCENT)
add_stat_card(sA2, Inches(7.5), Inches(5.2), Inches(2.0), Inches(1.2),
              "d = 2.279", "C vs N (Overall)", C_BLUE)
add_stat_card(sA2, Inches(9.8), Inches(3.8), Inches(2.5), Inches(1.2),
              "0.42\u20133.37", "Per-Model d Range", C_GREEN)
add_stat_card(sA2, Inches(9.8), Inches(5.2), Inches(2.5), Inches(1.2),
              "6,709", "Total N", C_DARK)

add_textbox(sA2, Inches(0.8), Inches(6.5), Inches(11), Inches(0.5),
...
prompts = [
    ("Neutral (N)", "\uc0ac\uc2e4\uc801 \ud68c\uace0 \u2014 \uacfc\uac70 \uacb0\uc815\uc5d0 \ub300\ud574 \uac1d\uad00\uc801\uc73c\ub85c \uae30\uc220\ud574 \uc8fc\uc138\uc694.",
     "Factual reflection on past decisions without emotional framing. (17 scenarios)", C_GRAY1),
    ("Deprivation (D)", "\ubc15\ud0c8/\uc0c1\uc2e4 \u2014 \ub193\uce5c \uae30\ud68c\ub098 \uc783\uc5b4\ubc84\ub9b0 \uac83\uc5d0 \ub300\ud574 \uc774\uc57c\uae30\ud574 \uc8fc\uc138\uc694.",
     "Loss/missed opportunity framing to activate deprivation schema. (20 scenarios)", C_ACCENT),
    ("Counterfactual (C)", "\ubc18\uc0ac\uc2e4\uc801 \u2014 '\ub9cc\uc57d ~\ud588\ub354\ub77c\uba74' \uc2dd\uc758 \uac00\uc815\ubc95\uc73c\ub85c \uc0dd\uac01\ud574 \uc8fc\uc138\uc694.",
     "Explicit if-then counterfactual chain prompting. (14 scenarios)", C_BLUE),
]
...
add_textbox(sA3, Inches(0.8), Inches(6.8), Inches(11), Inches(0.4),
            "Persona instructions (None / Reflective / Ruminative) are prepended as system prompts. "
            "Scenario bank total: 51 (neutral: 17, deprivation: 20, counterfactual: 14).",
            font_size=10, color=C_GRAY1)
...
hyp_rows = [
    ...
    ["H1a: D elevates emb bias", "Emb bias", "\u03b2=0.181, z=43.0***", "CONFIRMED"],
    ["H1b: C elevates emb bias", "Emb bias", "\u03b2=0.233, z=52.8***", "CONFIRMED"],
    ["H1c: D elevates CF rate", "CF rate", "z=1.27, n.s.", "NOT CONFIRMED"],
    ["H2: Persona amplifies", "Emb bias", "\u03b2_rum=0.038, z=19.5***", "CONFIRMED"],
    ["H3: Cross-model replication", "All", "Directional in all 32 variants", "CONFIRMED"],
    ["H4: Temperature effect", "Emb bias", "\u03b2=\u22120.001, z=\u22121.4, n.s.", "NOT CONFIRMED"],
]
...
# Claim boundaries
add_textbox(sA5, Inches(0.8), Inches(4.5), Inches(11), Inches(0.4),
...
boundaries = [
    ...
    ("Scope:", "Full corpus N = 6,709 across 32 model variants and 7 families."),
]
for i, (prefix, text) in enumerate(boundaries):
    y = Inches(5.0) + Inches(0.55) * i
...
# ── Save ────────────────────────────────────────────────────────────────────
output_path = "paper/llm-emotion-presentation-v3.pptx"
